...
//#region node_modules/.nitro/vite/services/ssr/assets/insights.functions-Dbf9hveG.js
var getFinancialInsights_createServerFn_handler = createServerRpc({
	id: "ad0f1c2f01aa2ac44bd00882e4cae283132eeec37762c981363819a13c8c3fda",
	name: "getFinancialInsights",
	filename: "src/lib/insights.functions.ts"
}, (opts) => getFinancialInsights.__executeServer(opts));
var getFinancialInsights = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((v) => v ?? {}).handler(getFinancialInsights_createServerFn_handler, async ({ data, context }) => {
	...
	const summary = {
		...
		top_categories: Object.entries(byCat).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([k, v]) => ({
			category: k,
			amount: v
		})),
		...
		logs_last_30_days: logs.filter((l) => {
			return new Date(l.logged_on) >= /* @__PURE__ */ new Date(Date.now() - 30 * 864e5);
		}).length
	};
	const apiKey = process.env.AI_API_KEY;
	if (!apiKey) return { insights: "AI insights are unavailable — AI_API_KEY is not configured." };
	...
	try {
		const res = await fetch(`${baseUrl}/chat/completions`, {
			...
			body: JSON.stringify({
				...
				messages: [{
					role: "system",
					content: "You are a helpful, concise personal finance and habit coach."
				}, {
					role: "user",
					content: prompt
				}]
		...
		return {
			insights: (await res.json()).choices?.[0]?.message?.content ?? "No insights returned.",
			summary
		};
	} catch (e) {
		return { insights: `Could not reach AI service: ${e instanceof Error ? e.message : "Unknown error"}` };
	}
});
//#endregion
export { getFinancialInsights_createServerFn_handler };
